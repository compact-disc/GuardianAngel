package com.guardianangel.xchart.retweets.admin;
...
public class AdminRetweetsXChartLineGraphBuilderSentimentPerMonth {
	...
	/**
	 * Constructor to create the image for the administrator for all the tweets
	 */
	public AdminRetweetsXChartLineGraphBuilderSentimentPerMonth() {
		...
		try {
			...
			BitmapEncoder.saveBitmap(chart, "./GuardianAngelUserData/UserXCharts/Admin_LineGraph_Sentiment_Per_Month_Retweets", BitmapFormat.PNG);
			
			CloudinaryUpload upload = new CloudinaryUpload("Admin_LineGraph_Sentiment_Per_Month_Retweets");
	...
	private int[] countSentimentByMonth(String currentSentiment) {
		...
		try {
			...
			data = statement.executeQuery("SELECT * FROM retweets");
