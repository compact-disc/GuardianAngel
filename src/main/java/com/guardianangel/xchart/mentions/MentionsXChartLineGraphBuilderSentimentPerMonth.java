package com.guardianangel.xchart.mentions;
 ...
 * 
 * Used to create line graphs for sentiment over time for the mentions data
 */
...
public class MentionsXChartLineGraphBuilderSentimentPerMonth {
	...
	/**
	 * 
	 * Creates the chart for sentiment per month for the user by their twitter id
	 * 
	 * @param twitterId
	 */
	public MentionsXChartLineGraphBuilderSentimentPerMonth(String twitterId) {
		...
		try {
			...
			BitmapEncoder.saveBitmap(chart, "./GuardianAngelUserData/UserXCharts/" + this.twitterId + "_LineGraph_Sentiment_Per_Month_Mentions", BitmapFormat.PNG);
			
			CloudinaryUpload upload = new CloudinaryUpload(this.twitterId + "_LineGraph_Sentiment_Per_Month_Mentions");
	...
	/**
	 * 
	 * Count the sentiment per month by current sentiment and return the array of integers
	 * 
	 * @param currentSentiment takes in the current sentiment to check
	 * @return
	 */
	private int[] countSentimentByMonth(String currentSentiment) {
		...
		try {
			...
			data = statement.executeQuery("SELECT * FROM mentions WHERE twitterid = " + "'" + this.twitterId + "'");
