package com.guardianangel.xchart.likes;
 ...
 * 
 * Used to create line graphs for sentiment over time for the likes data
 */
...
public class LikesXChartLineGraphBuilderSentimentPerMonth {
	...
	/**
	 * 
	 * Constructor that creates the image and takes in the users twitter id
	 * 
	 * @param twitterId
	 */
	public LikesXChartLineGraphBuilderSentimentPerMonth(String twitterId) {
		...
		try {
			...
			BitmapEncoder.saveBitmap(chart, "./GuardianAngelUserData/UserXCharts/" + this.twitterId + "_LineGraph_Sentiment_Per_Month_Likes", BitmapFormat.PNG);
			
			CloudinaryUpload upload = new CloudinaryUpload(this.twitterId + "_LineGraph_Sentiment_Per_Month_Likes");
	...
	/**
	 * 
	 * Returns the sentiment per month in a array of integers
	 * 
	 * @param currentSentiment takes in the current sentiment that is being checked for
	 * @return
	 */
	private int[] countSentimentByMonth(String currentSentiment) {
		...
		try {
			...
			data = statement.executeQuery("SELECT * FROM likes WHERE twitterid = " + "'" + this.twitterId + "'");
