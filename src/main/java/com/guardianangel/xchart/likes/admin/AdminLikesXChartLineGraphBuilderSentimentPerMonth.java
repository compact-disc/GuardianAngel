package com.guardianangel.xchart.likes.admin;
...
public class AdminLikesXChartLineGraphBuilderSentimentPerMonth {
	...
	/**
	 * Create the image for the administration page data for all tweets
	 */
	public AdminLikesXChartLineGraphBuilderSentimentPerMonth() {
		...
		try {
			...
			BitmapEncoder.saveBitmap(chart, "./GuardianAngelUserData/UserXCharts/Admin_LineGraph_Sentiment_Per_Month_Likes", BitmapFormat.PNG);
			
			CloudinaryUpload upload = new CloudinaryUpload("Admin_LineGraph_Sentiment_Per_Month_Likes");
	...
	/**
	 * 
	 * Counts the sentiment per month
	 * 
	 * @param currentSentiment takes in the current sentiment to check for
	 * @return
	 */
	private int[] countSentimentByMonth(String currentSentiment) {
		...
		try {
			...
			data = statement.executeQuery("SELECT * FROM likes");
