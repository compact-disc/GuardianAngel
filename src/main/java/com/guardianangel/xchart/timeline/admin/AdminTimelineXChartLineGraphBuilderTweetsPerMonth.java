package com.guardianangel.xchart.timeline.admin;
 ...
 * 
 * Used to create Line Graphs from timeline data using XChart
 */
...
public class AdminTimelineXChartLineGraphBuilderTweetsPerMonth {
	...
	/**
	 * Constructor to create the image for the administrators and all of the tweets for tweets per month
	 */
	public AdminTimelineXChartLineGraphBuilderTweetsPerMonth() {
		
		countTweetsByMonth();
		
		final XYChart chart = new XYChartBuilder().width(800).height(650).title("Tweets Per Month").build();
		...
		try {
			...
			BitmapEncoder.saveBitmap(chart, "./GuardianAngelUserData/UserXCharts/Admin_LineGraph_Tweets_Per_Month_Timeline", BitmapFormat.PNG);
			
			CloudinaryUpload upload = new CloudinaryUpload("Admin_LineGraph_Tweets_Per_Month_Timeline");
	...
	private void countTweetsByMonth() {
		...
		try {
			...
			data = statement.executeQuery("SELECT * FROM timeline");
